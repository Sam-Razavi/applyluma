...
export default function KPICard({
  ...
  const trendColor = trend == null || trend === 0 ? 'text-fg-subtle' : trend > 0 ? 'text-success-600' : 'text-danger-600'
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-danger-500/20 bg-chip-danger p-6 shadow-sm">
  ...
  return (
    <section
      className="group relative overflow-hidden rounded-lg border border-line bg-surface p-6 shadow-sm transition-all duration-200 ease-in-out hover:border-primary-500 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
      aria-label={`${title} metric`}
      tabIndex={0}
    >
      <div className="absolute right-4 top-4 rounded-full bg-surface p-2 transition-colors duration-200 group-hover:bg-primary-50">
        <Icon className="h-4 w-4 text-fg-subtle transition-colors duration-200 group-hover:opacity-80" aria-hidden="true" />
      </div>
      ...
      <div className="pr-10">
        ...
        <div className="mt-2 flex items-end justify-between gap-3">
          <span className="break-words text-3xl font-bold text-fg">
