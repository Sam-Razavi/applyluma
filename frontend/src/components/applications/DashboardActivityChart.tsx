...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-xl border border-indigo-100 bg-white px-4 py-3 shadow-xl">
      <p className="text-xs text-gray-400">{formatWeek(String(label))}</p>
      <p className="mt-0.5 text-lg font-bold text-indigo-600">
        {payload[0].value as number}
        <span className="ml-1 text-xs font-normal text-gray-500">applications</span>
      </p>
    </div>
  )
}
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-gray-400">Applications sent over the last 12 weeks</p>
        </div>
        <Link
      ...
      {/* Summary stats */}
      <div className="mb-5 grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-700">
        {[
      ...
      {loading ? (
        <div className="flex h-40 items-end gap-2 px-1">
          {[40, 65, 30, 80, 55, 90, 45, 70, 60, 85, 50, 75].map((h, i) => (
            <div
              key={i}
              className="flex-1 animate-pulse rounded-t-sm bg-indigo-100"
              style={{ height: `${h}%`, animationDelay: `${i * 60}ms` }}
            />
          ))}
        </div>
      ) : total === 0 ? (
        <div className="flex h-40 flex-col items-center justify-center gap-2 rounded-xl bg-gray-50 dark:bg-gray-900">
          <ArrowTrendingUpIcon className="h-8 w-8 text-gray-300" />
          <p className="text-xs text-gray-400">Start tracking applications to see your trend</p>
        </div>
      ) : (
        <div
      ...
      {/* Streak badge */}
      {streak >= 2 && (
        <div className="mt-4 flex items-center gap-1.5 rounded-xl bg-orange-50 px-3 py-2 dark:bg-orange-900/20">
