...
async def post_contact(
    ...
    *,
    bypass_token: bool = True,
) -> httpx.Response:
    if bypass_token and monkeypatch is not None:
        monkeypatch.setattr(contact_endpoint, "_verify_contact_token", lambda token: True)
...
async def test_submit_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    response = await post_contact(VALID_PAYLOAD, monkeypatch)
    assert response.status_code == 200
    assert response.json() == {"ok": True}


@pytest.mark.asyncio
async def test_submit_invalid_token(monkeypatch: pytest.MonkeyPatch) -> None:
    response = await post_contact(VALID_PAYLOAD, monkeypatch, bypass_token=False)
...
async def test_submit_honeypot_filled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    response = await post_contact(payload, monkeypatch)
    assert response.status_code == 400
